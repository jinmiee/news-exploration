...
stopwords = [
    ...
    '하기','해야지','하는거','하게','저걸','유승','기리','회견','뭔가','하기','빕니다',
]
...
def preprocess_text(text):
    # 텍스트가 dict인 경우 'comment' 필드에서 문자열을 추출
    if isinstance(text, dict):
        text = text.get('comment', '')  # 'comment' 필드가 없다면 빈 문자열로 처리

    # 텍스트가 아닌 경우 빈 문자열로 처리 (예: None, 숫자 등)
...
def remove_duplicate_documents():
    try:
        ...
        pipeline = [
            {
                "$group": {
                    "_id": "$video_url",  # URL을 기준으로 그룹화
                    "duplicates": {"$push": "$_id"},  # 중복된 문서의 ID를 저장
                    "count": {"$sum": 1}  # 문서 개수 계산
                }
            },
            {
                "$match": {"count": {"$gt": 1}}  # 중복된 문서가 1개 이상인 경우만 필터링
            }
        ]
        ...
        for duplicate in duplicates:
            ids = duplicate["duplicates"]  # 중복된 문서들의 ID
            # 하나만 남기고 나머지는 삭제
